...
def  SIFIF_func(X, N_var, Cor_var, list_var_exclude, training_index, results):
    ...
    Res = pd.DataFrame([], columns = ['const'] + list(X_without_list_var_exclude) +["R2_train", "R2adj_train", "AICc", "R2_test","R2adj_test","DeltaAICc","DeltaR2","modelid"])
    ...
    for model_var in results:
        ...
        Y_log_train = Y_log_train.reset_index()
        # drop the 'index' column
        X_in = X_in.drop(columns = ['index'])
        Y_log_train = Y_log_train.drop(columns = ['index'])
...
######################################################################
# Import ra data          
#X_WA = read_csv("/Users/rbani20/Documents/PostdocUW/proc_data/X_OR_lagged.csv")
X_WA = read_csv("/home/rbani20/projects/def-guichard/rbani20/postdoc/proc_data/X_OR_lagged.csv")    
X = X_WA
#######################################################################  
# Clean Data 
#list_col = list(X_WA)
...
training_sets = [[k+i for k in list(range(21))] for i in range(10)]
#######################################################################
# run the codes
...
if __name__ == '__main__':
    ...
    with open("/home/rbani20/projects/def-guichard/rbani20/postdoc/proc_data/comb_var_or_lagged.txt", "rb") as fp:
        results = pickle.load(fp)
    with open("/Users/rbani20/Documents/PostdocUW/proc_data/comb_var_or_lagged.txt", "rb") as fp:
        results = pickle.load(fp)
    
    pool = mp.Pool(2)
    
    Results1= [pool.apply_async(SIFIF_func, args =(X, 5, 0.75, ["Y","Y_SD","year"], training_index, results)) for training_index in training_sets[:2]]
    ...
    Results = pd.DataFrame(Results3, columns = ['const'] + list(X_without_list_var_exclude)+["R2_train", "R2adj_train", "AICc", "R2_test","R2adj_test","DeltaAICc","DeltaR2","modelid"])

    Results.to_csv("/home/rbani20/scratch/proc_data/OR_results_serial.csv", )
