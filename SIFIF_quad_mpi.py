...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 20 16:53:00 2021
...
def  SIFIF_func(X, list_var_exclude, training_percent, results):
    ...
    Res = pd.DataFrame([], columns = ['const'] + list(X_without_list_var_exclude)+
                       ["R2_train", "R2adj_train", "AICc", "R2_test","R2adj_test"])
    
    # go throughout the models 
    for model_var in results:
        if "meanCST_z3" in model_var:
            model_var = model_var + ("meanCST_z32",)
        if "meanCST_z2" in model_var:
            model_var = model_var + ("meanCST_z22",)
        if "meanCST_z1" in model_var:
            model_var = model_var + ("meanCST_z12",)
        if "meanLST_z4" in model_var:
            model_var = model_var + ("meanLST_z42",)
        if "meanLST_z3" in model_var:
            model_var = model_var + ("meanLST_z32",)
        if "meanLST_z2" in model_var:
            model_var = model_var + ("meanLST_z22",)
        if "meanLST_z1" in model_var:
            model_var = model_var + ("meanLST_z12",)
...
X['year']=list(range(1984,2014))

# Add quadratic 
X['meanCST_z12'] = X['meanCST_z1']**2 
X['meanCST_z22'] = X['meanCST_z2']**2 
X['meanCST_z32'] = X['meanCST_z3']**2 

X['meanLST_z12'] = X['meanLST_z1']**2
X['meanLST_z22'] = X['meanLST_z2']**2
X['meanLST_z32'] = X['meanLST_z3']**2
X['meanLST_z42'] = X['meanLST_z4']**2
...
if __name__ == '__main__':
    ...
    Results1= [pool.apply_async(SIFIF_func, args =(X, ["Y","Y_SD","year"], 0.8, results)) for i in range(0,10001)]
    
    Results2 = [R.get() for R in Results1]
    pool.close()
    pool.join()
    
    Results3 =[j for i in Results1 for j in i]
    ...
    Results.to_csv("/home/rbani20/projects/def-guichard/rbani20/postdoc/proc_data/WA_results_quad1000.csv", )
